...
class RobloxGroupCounter:
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    print(f"Error fetching group info: HTTP {response.status}")
                    return None
                    
        except Exception as e:
            print(f"Exception while fetching group info: {e}")
            return None
    ...
    async def send_member_update(self, group_info: Dict[Any, Any]) -> None:
        ...
        try:
            ...
            embed.set_thumbnail(url=f"https://www.roblox.com/group-thumbnails?params=[{{\"groupId\":{self.group_id},\"width\":150,\"height\":150,\"format\":\"png\"}}]")
            embed.set_footer(text="Roblox Group Monitor")
            
            await channel.send(embed=embed)
            print(f"Sent milestone notification for {current_count} members")
    ...
    async def send_regular_update(self, group_info: Dict[Any, Any]) -> None:
        ...
        try:
            ...
            message = f"We've reached **{current_count:,}** members! We are **{members_until_milestone:,}** away from **{next_milestone:,}**."
            
            await channel.send(message)
            print(f"Sent regular update for {current_count} members")
    ...
    async def check_member_count(self) -> None:
        ...
        if self.last_member_count is not None and current_count > self.last_member_count:
            # Members increased
            if self.should_celebrate_milestone(self.last_member_count, current_count):
                await self.send_member_update(group_info)
            else:
                # For regular increases, you might want to send less frequent updates
                # Uncomment the line below if you want updates for every member increase
                # await self.send_regular_update(group_info)
                pass
    ...
    async def start_monitoring(self) -> None:
        ...
        if self.is_running:
            print("Group monitoring is already running")
            return
            
        self.is_running = True
        print(f"Started monitoring Roblox group {self.group_id}")
        
        # Initial check to set baseline
        group_info = await self.get_group_info()
        if group_info:
            self.last_member_count = group_info.get("memberCount", 0)
            print(f"Initial member count: {self.last_member_count}")
    ...
    async def stop_monitoring(self) -> None:
        ...
        print("Stopped group monitoring")
