...
async def training_cancel_command(interaction: discord.Interaction, training_id: str):
    """Handle the training cancel command"""
    ...
    if training_data['host'] != interaction.user.id and not staff_role:
        await interaction.response.send_message("You can only cancel trainings that you started.", ephemeral=True)
        return
    ...
    embed = discord.Embed(
        title=f"<:Retail:1393269955260055702> Almora {training_data['type']} Training Cancelled",
        description=f"Our {training_data['type']} training has been cancelled, we're sorry for any inconveniences.",
        color=0x2b2d31
    )
    ...
    active_trainings[training_id]['status'] = 'cancelled'
    
    await interaction.response.send_message(f"Training {training_id} has been cancelled successfully!", ephemeral=True)

def setup(bot):
    @bot.tree.command(name="training_cancel", description="Cancel an active training session")
    @app_commands.describe(
        training_id="Training ID to cancel (generated when starting a session)"
    )
    async def training_cancel(
        interaction: discord.Interaction, 
        training_id: str
    ):
        await training_cancel_command(interaction, training_id)