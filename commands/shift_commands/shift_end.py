...
def setup(bot):
    ...
    @app_commands.describe(shift_id="The ID of the shift to end")
    async def shift_end(interaction: discord.Interaction, shift_id: str):
        ...
        if shift_data['host'] != interaction.user.id and not staff_role:
            await interaction.response.send_message("You can only end shifts that you started.", ephemeral=True)
            return
        ...
        # Create ended embed
        embed = discord.Embed(
            title="<:Shifts:1393545049832292482>  Almora Shift Ended",
            description=f"Our {shift_data['end_time']} shift has now ended, thank you to everyone who\nattended.",
            color=0x4A4A4A
        )
        ...
        # Update shift status
        active_shifts[shift_id]['status'] = 'ended'
        
        await interaction.response.send_message(f"Shift {shift_id} has been ended successfully!", ephemeral=True)